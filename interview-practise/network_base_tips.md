...
+ **TCP特点**：
    ...
    + **拥塞控制**，TCP 发送方也可能因为 IP 网络的堵塞而被遏制。**拥塞控制方法** 有：**端到端拥塞控制**（通过 TCP 报文段的丢失作为判断拥塞的根据，TCP 会相应地减少其窗口的长度，或者增加往返的时延值）、**网络辅助的拥塞控制**（由路由器向发送方提供关于网络中拥塞状态的显式反馈信息，路由器会显示地通知发送方能在输出链路上支持的最大主机发送速率）。
