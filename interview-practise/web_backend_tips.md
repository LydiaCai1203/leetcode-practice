...
```markdown
1. 高并发（时间极短、瞬间用户量大，可能出现缓存雪崩、缓存击穿、缓存穿透）
2. 超卖    
3. 恶意请求（机器模拟人瞬间十几万请求然后倒卖掉）    
4. 链接暴露    
5. 每秒上万甚至十几万的 QPS 如果直接打到数据库，基本把库打挂。如果这个库还会涉及到别的业务，没做限流、降级、熔断，导致别的业务挂了就完了
```

**解决方案：**

```markdown
1. 服务单一职责（微服务设计思想，分布式的部署方式）
下订单有一个下订单的服务，用户登录管理有个用户服务，秒杀也单独放一个服务里，单独给它建一个数据库。单一职责的好处就是，就算秒杀没有抗住，秒杀库崩了，服务也挂了，也不会影响别的服务。

2. 秒杀链接加盐
把 秒杀 URL 动态化，这样连写代码的人自己都不知道。比如通过 MD5 之类的加密算法加密随机的字符串去做 URL，然后通过前端代码获取 URL 后台校验才能通过。

3. Redis 集群
单机的 Redis 扛不住，就可以利用 Redis 的集群模式，这里又涉及到 Redis 集群、主从同步、读写分离、哨兵模式、持久化 等等的知识点。

4. Nginx
使用 Nginx 做七层负载均衡(之类甚至是不是还能说一下四层负载均衡)，恶意请求拦截也需要用到它，一般单个用户请求此时太夸张，不像人为的请求在网关那一层就可以拦截掉，不然万一把服务器打崩、缓存击穿等问题就出现了。

5. 资源静态化
静态资源可以放到 cdn 服务器里面，把所有能提升效率的步骤都做一下，减少真正秒杀时服务器的压力。

6. 按钮控制
秒杀时间点之前按钮置灰不能点击，或者点击之后置灰几秒，不让连续点击。

7. 限流
前端限流：就是前面说的按钮控制。
后端限流：
  a. 库存预热：秒杀的本质是对库存的抢夺，每个秒杀的用户来都要去数据库校验库存，扣减库存，数据库可能顶不住。可以借助 Redis，在秒杀开始前，通过定时任务或者运维同学提前把商品的库存加载到 Redis 中，让整个校验流程去 Redis 里做，真要减库存了再去修改库存即可。不过可能出现高并发的问题。

8. Lua
Lua 脚本是 Redis2.6 的亮点。解决了 Redis 长久依赖不能高效处理 CAS(check-and-set) 的缺点，可以组合使用多个命令。类似 Redis 的事务，不会被其它的命令插队，可以完成一些 Redis 事务性的操作。这样可以把判断库存和扣减库存的操作放在一个脚本丢给 Redis 去做。

9. 削峰填谷
把请求放到 mq 里面，一点点消费去改库存。这里要注意一下分布式一致性把我觉得。
```
