...
--------------------
python到底是强类型的语言还是弱类型的语言
因为python不会自动在数字和字符串之间进行进行强制转换
1. 很少隐式转换类型，说明它是强类型语言，如果经常这么做就是弱类型语言，java,c++,python都是强类型语言，php,js是弱类型语言
2. 在编译时检查类型的语言是静态类型语言，在运行时检查类型的语言是动态类型语言，动态类型便于代码重用，代码行数更少，python
是动态强类型语言
