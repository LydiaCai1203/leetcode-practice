...
![MySQL逻辑架构图](/Users/caiqingjing/Desktop/MySQL逻辑架构图.png)
...
+ **AutoCommit(MySQL 默认)**
  ...
  也就是说当禁用自动提交，则需要显示执行 commit 或 rollback，该事务才算是结束。
...
+ **隐式和显式锁定**
  ...
  InnoDB 采用的是*两阶段锁定协议 (two-phase locking protocol)*，锁只有在 commit 或 rollback 的时候才会被释放，并且所有的锁都是同一时刻被释放。
...
+ InnoDB 的数据存储在表空间(table space)中，由一系列的数据文件组成。
