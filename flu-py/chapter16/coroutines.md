...
*首先我们要知道，为了执行某些操作，必须排定协程的运行时间，然后使用 asyncio.Task 对象包装协程。*

**`asyncio.async(coro_or_future, *, loop=None)`**

```markdown
如果 coro_or_future 是 Future 或 Task 对象，就原封不动地返回，如果是协程，就会调用 loop.create_task(...) 创建 Task 对象。loop 关键字是可选的，用于传入事件循环对象。如果没有传入，就调用 asyncio.get_event_loop() 来获取循环对象。
```

**`BaseEventLoop.create_task(coro)`**

```markdown
这个方法排定协程的执行时间，返回一个 Task 对象。
```

**协程、Future、任务之间的关系是：**

[参考文档](https://docs.python.org/3/library/asyncio-task.html)

```markdown
协程：
协程函数：一个用 async def 定义的函数
协程对象：协程函数的返回对象。

Task:
Task 被用于并发调度协程，当一个协程对象被 Task 包裹，比如：async.create_task(coro_obj)。相当于被排期了，排期以后协程函数将会很快被执行到。

Future:
Future是一个特殊的低级别等待对象，代表异步操作的最终结果。当一个 Future 对象是 awaited 状态，这意味着协程会处于等待状态，直到 Future 在其他地方被处理了。

上面三种对象都能被 yield from 驱动。
```

### 18.2 使用 asyncio 和 aiohttp 下载
...
def download_one(cc):
    ...
    # 注意这里在还是会阻塞的，阻塞主线程就相当于阻塞了事件循环
    save_flag(image, f'{cc}.gif')
    return cc

def download_many(cc_list):
    # 并不是创建事件循环，而是获取事件循环的引用
    loop = asyncio.get_event_loop()
    to_do = [download_one(cc) for cc in cc_list]
    # 虽然函数的名称是 wait, 但不是阻塞型函数，wait 是一个协程，等尺寸传给它的所有协程运行完毕以后就会结束。
    wait_coro = asyncio.wait(to_do)
    # 驱动 task 对象
    res, _ = loop.run_utin_complete(wait_coro)
    # 如果没有引用就进行关闭会报错
    loop.close()
    return len(res)    
```
...
```markdown
wait() 的参数是一个由 Future 或 coroutine 构成的可迭代对象。作用是分别把各个协程对象包装进 Task 对象中。因为 wait 是一个协程函数，所以 wait_coro 是一个协程对象。为了驱动协程，将其传给 loop.run_until_complete(...) 方法。wait 函数有两个关键字参数：timeout 和 return_when, 设定了可能会返回未结束的期物。
```

+ **`loop.run_until_complete(...)`**

```markdown
run_until_complete(...) 的参数是一个 Future 或 Coroutine。如果是协程，则作用和 wait 函数一样，包进 Task 对象中。在本例中，run_until_complete 会对传入的 task 对象使用 yield from 进行驱动。返回的第一个元素是一系列结束的 Future, 第二元素始终为空。
```
...
def get_flag(base_url, cc):
    ...
    if resp.status == 200:
        ...
        raise aiohtto.HttpProcessError(
            code=resp.statys,
            messahe=resp.reason,
...
def download_one(cc, base_url, semaphore):
    try:
        # 退出 with 语句后，semaphore 的值会递减，保证任何时候都不会超过 req_count 个 get_flag 协程启动
        with (yield from semaphore):
            image = yield frm get_flag(base_url, cc)
    except web.HTTPNotFound:
        status = HTTPStatus.not_found
        msg = 'not found'
    else:
        save_flag(imag, cc.lower() + '.gif')
        status = HTTPStatus.ok
        msg = 'ok'
    return (status, cc)

@asyncio.coroutine
def download_coro(cc_list, base_url, concur_req):
    counter = collections.Counter()
    semaphore = asyncio.Semaphore(concur_req)
    to_do = [
        download_one(cc, base_url, semaphore) 
        for cc in sorted(cc_list)
    ]
    # 产出 future 的结果，只要有 future 有结果就会返回
...
def download_many(cc_list, base_url, concur_req):
    loop = asyncio.get_event_loop()
    coro = downloader_coro(cc_list, base_url, concur_req)
...
+ `semaphore` 
  ...
  ```markdown
  维护着一个内部计数器，若在对象上调用 .acquire() 协程方法，计数器则会递减。若在对象上调用 .release() 协程方法，计数器则会递增。计数器的初始值在实例化 Semaphore 的时候进行设定。
  ```

### 18.4 使用 Executor 对象，防止阻塞事件循环
...
+ `run_in_exector`
  ...
  ```markdown
  asyncio 的事件循环背后维护着一个 ThreadPoolExecutor 对象，因此我们可以调用 run_in_executor 方法，把可调用的对象发给它执行。
  ```
