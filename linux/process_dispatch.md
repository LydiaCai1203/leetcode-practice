...
[CFS 出现的原因](https://blog.51cto.com/dog250/1274147)

**O(1)**

```markdown
Linux 2.5 系列内核中，采用了一种叫做 O(1) 调度程序的新调度程序。O(1) 调度程序中有两个队列：运行队列、过期队列。比如有两个优先级一样的进程，系统依照优先级分配时间片，时间片用完的进程会进入到过期队列，只有当运行队列中的所有进程都进入到过期队列中，才会将运行队列和过期队列进行交换。这样对于单个进程来说，执行就是断断续续的，需要等待的时间就会很长。因此这个算法只能保证运行队列中的进程的公平性，但是不能保证过期队列中进程的公平性。

优点：
O(1) 调度器在数以十计的多处理器环境的环境下尚能表现出近乎完美的性能和可扩展性。

缺点：
但是在有很多交互程序要运行的桌面系统上则表现不佳。
```
...
CFS 的本质就是用虚拟时钟替代了硬件时钟，每个进程都有一个虚拟时钟，虚拟时钟的快慢依据进程的优先级决定。优先级越高的进程的虚拟时钟走的越慢，优先级越低的进程的虚拟时钟走的越快。调度模块选择虚拟时钟走的最慢的进程执行，这就是公平的含义。

每次系统时钟中断都会导致当前执行进程出队,然后调度器会从红黑树中找出下一个要执行的进程，如果还是当前进程的话，再将其入队。CFS 的细节实现之后再看。
```
...
完美调度器的目标是：
如果有两个进程，1个处理器，那么希望两个进程各自占用 50% 的处理器资源。也就是所谓的并行运行。但是显然这是不可能的，因此只有两个进程频繁切换，也就是说分给两个进程的时间片非常非常小，才能接近各占 50% 的目标。

CFS：
CFS 最初给 N 个进程分别分配 1/N 的处理器时间。然后 CFS通 过优先级（nice值）权衡每个进程的比例，调整分配。默认的优先级为 0，权值是 1，则比例不变。nice 设置的越小（优先级越高），权值就越高，就增加给该进程的处理器比例值；nice 设置的越高（优先级越低），权值越低，就减少分配给该进程的比例值。
```

**目标延迟**

```markdown
CFS就基于权值分配给了每个进程处理器比例。要确定每个进程真正的执行时间，CFS需要把比例划分为一个固定周期，这个周期叫目标延迟，表示系统的调度延迟。越小的调度周期带来越好的交互性，同时更接近完美的多任务。但是也必须承受更高的切换代价和更差的系统总吞吐能力。假如 目标延迟 为 20ms, 假如有 4个任务，每个只能运行 5ms, 假如有 20个任务，每个只能运行 1ms。
```
...
抢占:
内核提供了一个 `need_resched` 标志表明是否需要重新执行一次调度。当某个进程应该被抢占时，`scheduler_tick()` 就会设置这个标志：当一个优先级高的进程进入可执行状态时，`try_to_wake_up()` 也会设置这个标志，然后内核会检查这个标志，确认其被设置后就会调用 `schedule()` 切换到一个新的进程。

在返回到用户空间以及中断返回的时候，内核也会检查 `need_resched` 标志，如果已被设置，内核会继续执行之前去调用 `schedule()`。这个变量被存储在每个进程的 `struct task_struct` 中。
```

#### 4.6.1 用户抢占

```markdown
内核即将返回用户空间的时候，如果 `need_resched` 标志被设置了，会导致 `schedule()` 被调用，此刻就会发生用户抢占。
...
```markdown
内核抢占就是，只要重新调度是安全的(只要没有持有锁)，内核可以在任何时间抢占正在执行的任务。Linux 2.6 中，引入内核抢占。
在不支持内核抢占的内核中，内核代码可以一直执行，直到它完成为止。也就是说，调度程序没有办法在一个内核级的任务正在执行的时候重新调度。

关于锁：
Linux 为每个进程的 `thread_info` 引入了 `preempt_count` 计数器。该计数器的初始值为 0，每次用到锁的时候数值加 1，释放锁的时候数值减 1。因此当数值为 0 的时候，意味允许内核执行抢占。

如果内核中的进程被阻塞了，或是它显式地调用了 `schedule()`，内核抢占也会显式地发生。这种形式的内核抢占，不需要检查 `need_resched` 和 `preempt_count`，内核支持这种行为。

内核抢占发生在：
1. 中断处理正在执行，且返回内核空间之前。
2. 内核代码再一次具有可抢占性的时候。
3. 内核中的任务显式地调用 `schedule()`。
4. 内核中的任务阻塞(这样会导致调用 `schedule()`)。
```

### 4.7 实时调度策略

```markdown
借助调度类的框架，这些实时调度策略并不由完全公平调度器所管理，而是被一个特殊的实时调度器来管理。普通的、非实时的调度策略是 `SCHED_NORMAL`。这两种实时算法实现的都是静态优先级，内核不为实时进程计算动态优先级，这能保证给定优先级别的实时进程总是能抢占优先级比它低的进程。
```

**SCHED_FIFO**

```markdown
处于可运行状态的 `SCHED_FIFO` 级进程 会比任何 `SCHED_NORMAL` 级进程都先得到调度。一旦一个 `SCHED_FIFO` 级进程处于可执行状态，就会一直执行下去，直到它自己阻塞了，或是它显示地释放处理器为止。只有更高优先级的 `SCHED_FIFO` 或 `SCHED_RR` 才能抢占。如果有两个同级别的进程，则会轮流进行。
```

**SCHED_RR**

```markdown
`SCHED_RR` 与 `SCHED_FIFO` 大体相同。只是 `SCHED_RR` 在耗尽事先分配给它的时间后就不能再继续执行了。耗尽以后，其它同一优先级的实时进程会被轮流调度。高优先级进程总是立即抢占低优先级，但是低优先级进程绝不能抢占 `SCHED_RR` 任务，即使它的时间片已经耗尽。
```
...
自己画的脑图，但是总感觉画的不是很好。后面回头复习的时候再总结吧。

![](/Users/caiqingjing/project/private/leetcode-practice/statics/进程调度.png)s
